...
const FamiliasTable: React.FC<FamiliasTableProps> = ({
  families,
  onDelete,
  navigate,
}) => (
      ...
      <tbody>
        {families.map((item) => (
          <tr key={item.id}>
            ...
            <td>
              {item.status.toLowerCase() === BenefStatus.NECESSITA_ATENCAO && (
                <RiErrorWarningLine
                  style={{ color: "#f17342", width: "4rem", height: "2rem" }}
                />
              )}
              <button
              ...
              <button
                ...
                className="table-btn excluir-btn"
              >
                EXCLUIR
              </button>
            </td>
