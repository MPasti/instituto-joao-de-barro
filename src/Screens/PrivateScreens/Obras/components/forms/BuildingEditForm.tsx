...
const validationSchema = object({
    ...
    situacao_construcao: string().required("Situação da construção é obrigatória").oneOf(
        ["Em andamento", "Concluída", "Pausada", "Cancelada"], "Situação inválida"
    ),
...
export const BuildingEditForm = ({ selectedBuilding }: IEditFormProps) => {
    const { register, handleSubmit, formState: { errors } } = useForm<EditFormData>({
        ...
        defaultValues: {
            ...
            dt_inicio: selectedBuilding.dt_inicio ? new Date(selectedBuilding.dt_inicio).toISOString().split("T")[0] : '',
            dt_termino: selectedBuilding.dt_termino ? new Date(selectedBuilding.dt_termino).toISOString().split("T")[0] : '',
